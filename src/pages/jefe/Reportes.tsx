...
interface VisitRecord {
  ...
  client: { name: string }; employee: { name: string };
}
...
export default function JefeReportes() {
  ...
  useEffect(() => { usersApi.getUsers('EMPLEADO').then(setUsers); }, []);

  const loadData = () => {
    setLoading(true);
    Promise.all([
      attendanceApi.getAllAttendance(filterUser || undefined, from, to),
      visitsApi.getAllVisits(undefined, filterUser || undefined),
  ...
  return (
    <div className="flex flex-col gap-4 max-w-7xl mx-auto">
      <div className="border-b border-border-subtle/10 pb-4">
        <h1 className="text-2xl font-bold text-on-surface">Reportes Detallados</h1>
        <p className="text-sm text-on-surface-variant mt-1">Análisis de asistencia y visitas del equipo</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Jornadas Completas', value: attendance.filter((a) => a.status === 'COMPLETED').length, color: 'text-role-supervisor' },
        ].map((s) => (
      ...
      <div className="flex gap-1 bg-surface-container rounded-xl p-1 w-fit">
        {(['asistencia', 'visitas'] as const).map((t) => (
          ...
          >
            {t === 'asistencia' ? 'Asistencia' : 'Visitas'}
          </button>
        ))}
      </div>
              ...
              <tbody className="divide-y divide-outline-variant/20">
                {attendance.map((a) => (
                  <tr key={a.id} className="hover:bg-surface-container-low transition-colors">
                    ...
                    <td className="px-4 py-3 font-mono text-status-late">{fmt(null)}</td>
                    <td className="px-4 py-3 font-mono text-secondary">{fmt(null)}</td>
              ...
              <tbody className="divide-y divide-outline-variant/20">
                {visits.map((v) => (
                  <tr key={v.id} className="hover:bg-surface-container-low transition-colors">
                    ...
                    <td className="px-4 py-3 text-on-surface">{v.client.name}</td>
